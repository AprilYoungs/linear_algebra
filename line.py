...
class Line(object):
    ...
    def intersection(self, line):
        '''find the intersection point if there's any,
            raise exception with the reason why there's
            no intersection point
        '''
        if not isinstance(self.basepoint, Vector) or not isinstance(line.basepoint, Vector):
            raise Exception('not a line')

        a1, b1 = self.normal_vector.coordinates
        k1 = self.constant_term
        a2, b2 = line.normal_vector.coordinates
        ...
        isParallel = False
        isSameLine = False
        if MyDecimal(xm).is_near_zero():
            if MyDecimal(xz).is_near_zero():
                isSameLine = True
            isParallel = True
        ...
        inter = None if isParallel else Vector([x, y])

        return inter, isParallel, isSameLine


# 静态方法，类似class方法，但是不可以使用跟class相关的东西
    @staticmethod
...
print(line5.intersection(line6)[0])